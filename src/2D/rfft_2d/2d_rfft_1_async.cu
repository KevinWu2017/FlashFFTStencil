    ...
    double *output)
{
    ...
    const int input_idx = intput_idx_x * ACTUAL_WIDTH + intput_idx_y;
    ...
    //     // real_shared[row * unit + col] = input[input_idx + row * ACTUAL_WIDTH + col];
    //     // imag_shared[row * unit + col] = input[input_idx + unit + row * ACTUAL_WIDTH + col];

    //     int dst_real = __cvta_generic_to_shared(real_shared + i);
    //     int dst_imag = __cvta_generic_to_shared(imag_shared + i);

    //     asm volatile("cp.async.ca.shared.global [%0], [%1], 8;\n"
    //                     :
    //                     : "r"(dst_real), "l"(&input[input_idx + row * ACTUAL_WIDTH + col])
    //                     : "memory");
    //     asm volatile("cp.async.ca.shared.global [%0], [%1], 8;\n"
    //                     :
    //                     : "r"(dst_imag), "l"(&input[input_idx + unit + row * ACTUAL_WIDTH + col])
    //                     : "memory");

    //     // asm volatile("cp.async.cg.shared.global{.level::cache_hint}{.level::prefetch_size} [%0], [%1], 8{, cache-policy};\n"
    //     //                 :
    //     //                 : "r"(dst_imag), "l"(&input[input_idx + unit + row * ACTUAL_WIDTH + col])
    //     //                 : "memory");
    ...
    // for (size_t i = laneId; i < rfft_size; i += WARP_SIZE)
    {
        ...
        int dst_real = __cvta_generic_to_shared(reinterpret_cast<double2 *>(real_shared)) + laneId * sizeof(double2);
        int dst_imag = __cvta_generic_to_shared(reinterpret_cast<double2 *>(imag_shared)) + laneId * sizeof(double2);

        asm volatile("cp.async.cg.shared.global.L2::256B [%0], [%1], 16;\n"
                     :
                     : "r"(dst_real), "l"(&input[input_idx + row * ACTUAL_WIDTH + col * 2])
                     : "memory");
        asm volatile("cp.async.cg.shared.global.L2::256B [%0], [%1], 16;\n"
                     :
                     : "r"(dst_imag), "l"(&input[input_idx + unit + row * ACTUAL_WIDTH + col * 2])
                     : "memory");
...
#pragma unroll
    ...
    {
        // wmma::load_matrix_sync(b_frag_imag, input + input_idx + unit + i * WMMA_K * ACTUAL_WIDTH, ACTUAL_WIDTH); // d
        wmma::load_matrix_sync(b_frag_imag, imag_shared + i * WMMA_K * unit, unit); // d
...
#pragma unroll
    for (size_t i = 0; i < 2; i++)
    {
        // wmma::load_matrix_sync(b_frag_real, input + input_idx + i * WMMA_K * ACTUAL_WIDTH, ACTUAL_WIDTH); // c
        wmma::load_matrix_sync(b_frag_real, real_shared + i * WMMA_K * unit, unit); // c
...
#pragma unroll
    for (size_t i = 0; i < 2; i++)
    {
        wmma::load_matrix_sync(a_frag_imag, imag_shared + WMMA_K * i, unit); // b
...
#pragma unroll
    for (size_t i = 0; i < 2; i++)
    {
        wmma::load_matrix_sync(a_frag_real, real_shared + WMMA_K * i, unit); // a
