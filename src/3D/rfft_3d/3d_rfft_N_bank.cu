                                ...
                                double *output)
{
    ...
    for (size_t i = laneId; i < rfft_size; i += WARP_SIZE)
    {
        ...
        real_shared[x * bank_unit_unit + y * bank_unit + z] = input[input_idx_real + x * (ACTUAL_WIDTH * ACTUAL_WIDTH) + y * ACTUAL_WIDTH + z];
        imag_shared[x * bank_unit_unit + y * bank_unit + z] = input[input_idx_imag + x * (ACTUAL_WIDTH * ACTUAL_WIDTH) + y * ACTUAL_WIDTH + z];
    }
...
#pragma unroll
        ...
        {
            wmma::load_matrix_sync(b_frag_imag[j], p_imag + j * WMMA_K * bank_unit_unit, bank_unit_unit); // d
            ...
            wmma::load_matrix_sync(b_frag_real[j], p_real + j * WMMA_K * bank_unit_unit, bank_unit_unit); // c
...
#pragma unroll
        ...
        wmma::store_matrix_sync(p_real, fft_real, bank_unit_unit, wmma::mem_row_major);
        wmma::store_matrix_sync(p_imag, fft_imag, bank_unit_unit, wmma::mem_row_major);
    }

#pragma unroll
    for (size_t i = 0; i < unit; i++)
    {
        double *p_date_real = real_shared + i * bank_unit_unit;
        double *p_date_imag = imag_shared + i * bank_unit_unit;
...
#pragma unroll
        for (size_t j = 0; j < 2; j++)
        {
            wmma::load_matrix_sync(b_frag_imag[j], p_date_imag + j * WMMA_K * bank_unit, bank_unit); // d
            ...
            wmma::load_matrix_sync(b_frag_real[j], p_date_real + j * WMMA_K * bank_unit, bank_unit); // c
...
#pragma unroll
        ...
        wmma::store_matrix_sync(p_date_real, kf_real, bank_unit, wmma::mem_row_major);
        wmma::store_matrix_sync(p_date_imag, kf_imag, bank_unit, wmma::mem_row_major);
