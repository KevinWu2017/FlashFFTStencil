    ...
    double *output)
{
    ...
    const int intput_idx_y = blockIdx.x * 2 * nwarp_in_block * unit;

    const int input_idx = intput_idx_x * ACTUAL_WIDTH + intput_idx_y + warp_id * 2 * unit;
...
#pragma unroll
    ...
    const int output_idx_y = blockIdx.x * 2 * nwarp_in_block * sub_input_width + warp_id * 2 * sub_input_width;
    ...
    else if ((fft_id_x == gridDim.x - 1) && (fft_id_y != gridDim.y - 1))
    {
        for (size_t i = laneId; i < rfft_size; i += WARP_SIZE)
        {
            ...
            const int idx_x = (output_idx_x + row);
            const int idx_y_real = (output_idx_y + col);
            const int idx_y_imag = ((col >= sub_input_width) && (warp_id == nwarp_in_block -1)) ? (col - sub_input_width) : (output_idx_y + sub_input_width + col);
