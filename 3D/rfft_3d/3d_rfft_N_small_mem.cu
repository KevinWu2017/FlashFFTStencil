                                ...
                                double *output)
{
    ...
    const int intput_idx_x = blockIdx.x * sub_input_width;
    const int intput_idx_y = blockIdx.y * sub_input_width;
    const int intput_idx_z = blockIdx.z * sub_input_width * 2;

    const int input_idx_real = intput_idx_x * INPUT_WIDTH * INPUT_WIDTH + intput_idx_y * INPUT_WIDTH + intput_idx_z;
    const int input_idx_imag = input_idx_real + sub_input_width;
    ...
    for (size_t i = laneId; i < rfft_size; i += WARP_SIZE)
    {
        ...
        real_shared[x * (unit * unit) + y * unit + z] = input[input_idx_real + x * (INPUT_WIDTH * INPUT_WIDTH) + y * INPUT_WIDTH + z];
        imag_shared[x * (unit * unit) + y * unit + z] = input[input_idx_imag + x * (INPUT_WIDTH * INPUT_WIDTH) + y * INPUT_WIDTH + z];
    }
